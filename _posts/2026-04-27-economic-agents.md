...
Meanwhile, the capabilities of frontier models have been climbing rapidly. In high-stakes domains like medicine, open-ended physician-written evaluations (HealthBench) show model performance improving from 16% (GPT-3.5) to 32% (GPT-4) and up to 60% with the latest models <d-cite key="Arora2025"></d-cite> within two years – a nearly 4x improvement. In software engineering, new benchmarks of freelance programming tasks (SWE-Lancer) valued at &dollar 1M found that state-of-the-art models can now complete a significant subset of real-world coding jobs (earning about &dollar400k of the &dollar1M total value) <d-cite key="Miserendino2025"></d-cite>, though they still fail on the majority of tasks. We are entering what Silver and Sutton term the *Era of Experience* <d-cite key="SilverSutton2025"></d-cite>, where AI agents learn continuously from real-world interactions rather than solely from static training data.
...
2.  **Dollar Valuation.**
    * *Software*: Upwork median payout $\tilde{p}=$ &dollar 500; full ticket pool &dollar 1M <d-cite key="Miserendino2025"></d-cite>.
    * *Healthcare*: average avoided readmission saves &dollar 12,000 (CMS FY-24); multiply HealthBench rubric score by that coefficient <d-cite key="Arora2025"></d-cite>.
...
The dataset comprises 1,488 Upwork tickets totalling &dollar 1M in payouts. Claude 3.5 Sonnet earns &dollar 403k, yielding a median cost–saving rate (CSR) of roughly &dollar 300 and an HIR of about 74% <d-cite key="Miserendino2025"></d-cite>. Robust human–written tests mitigate grader gaming.

### HealthBench: Safety–Critical Triage
GPT–4o scores 32%, whereas a later model (*o3*) scores 60% <d-cite key="Arora2025"></d-cite>. Using the &dollar 12,000 valuation coefficient (Section 4), GPT–4o's 32\% rubric score translates to a CSR of approximately &dollar 3,840 per triage (0.32 $\times$ &dollar 12,000). The *o3* model, with a 60% score, yields an HIR of roughly 40\%.
...
* **Baseline (Human-Only)**: 2 analysts, 10 days (160 hours), &dollar 75/hour = &dollar 12,000 per report.
* **Output**: Maximum 2 reports per month per team.

**AI Fundamentals Metrics**

*Cost Saving*
* AI processing: &dollar 500
* Human review (reduced to 40 hours): &dollar 3,000
* Total agent-assisted cost: &dollar 3,500
* **Cost Saving** = &dollar 12,000 - &dollar 3,500 = **&dollar 8,500 per report**

*Efficiency Premium*
* Increased throughput (4 reports/month vs. 2): &dollar 10,000 per report
* Enhanced quality/scope: &dollar 1,000 per report
* **Efficiency Premium** = **&dollar 11,000 per report**
...
* **Intervention penalty**: We assign a governance penalty of &dollar 10 per basis point of intervention; hence 450 b.p. =4.5\% $\times$ 10 = &dollar 4,500.

*Agent Economic Value (AEV)*
* Additional costs (infrastructure, exploration): &dollar 1,000
* Intervention penalty: &dollar 10/b.p. × 450 b.p. = &dollar 4,500 (reduced from &dollar 9,500)
* **AEV** = &dollar 8,500 + &dollar 11,000 - (&dollar 1,000 + &dollar 4,500) = **&dollar 14,000 per report**

**Key Outcomes**
The AI agent delivers substantial value with &dollar 14,000 net profit per report. Even with a 5% intervention rate, the system achieves:
* 71% cost reduction (&dollar 8,500 savings per report)
* 100% increased throughput (4 vs. 2 reports monthly)
* 117% ROI (&dollar 14,000 profit/&dollar 12,000 baseline cost)

Further reducing intervention incidents to 2% would add another &dollar 3,000 to AEV (to &dollar 17,000), highlighting how reliability directly impacts profitability. The growing data asset of analyzed reports and market insights can be capitalized once it demonstrably improves future cash flows.
...
* **Baseline**: 100 agents handling 50,000 monthly interactions at &dollar 10 per interaction (&dollar 500,000 total)
...
* AI system cost: &dollar 50,000
* Reduced human staffing: 50 agents at &dollar 5,000 each = &dollar 250,000
* **Cost Saving** = &dollar 500,000 - &dollar 300,000 = **&dollar 200,000 per month**

*Efficiency Premium*
* CSAT improvement (70\% → 88\%), reducing churn: &dollar 120,000
* 24/7 availability: &dollar 20,000
* **Efficiency Premium** = **&dollar 140,000 per month** (increased from previous &dollar 120,000)

*Human Intervention Cost*
* 600 interactions (3% of AI-only attempts) require human rescue (reduced from 5%)
* **Intervention penalty**: We assign a governance penalty of &dollar 50 per basis point of intervention; hence 200 b.p. = 2\% $\times$ &dollar 50 = &dollar 10,000.

*Agent Economic Value (AEV)*
* Intervention penalty: &dollar 50/b.p. $\times$ 200 b.p. = &dollar 10,000 (reduced from &dollar 20,000)
* **AEV** = &dollar 200,000 + &dollar 140,000 - (&dollar 0 + &dollar 10,000) = **&dollar 330,000 per month**

**Key Outcomes**
The AI support system delivers exceptional value with &dollar 330,000 monthly profit (66% return). Key results include:
* 40% cost reduction (&dollar 200,000 monthly savings)
...
Further reducing the intervention rate from 3% to 1% would add another &dollar 10,000 to monthly AEV, completely eliminating the penalty. The customer interaction data asset grows continuously, becoming increasingly valuable for personalization and service improvement.
...
* **Baseline (Human-Only):** 1 senior analyst + 2 associates, 80 hours total at an average blended rate of &dollar 180/h $\Rightarrow$ &dollar 14,400 per report.
* **Throughput:** 10 tickers per quarter.

**AI Fundamentals Metrics**

*Cost Saving*
* LLM inference and retrieval cost: &dollar 400.  
* Human review (senior analyst 8 h): &dollar 1,440.
* Total agent-assisted cost: &dollar 1,840.
* **Cost Saving** = &dollar 14,400 - &dollar 1,840 = &dollar 12,560 per report.

*Efficiency Premium*
* Faster turn-around enables coverage of 25 tickers/quarter (2.5× baseline) generating incremental fee income of &dollar 30,000.
* Richer alt-data synthesis improves hit-rate, adding expected alpha worth &dollar 5,000 per report.
* **Efficiency Premium** = &dollar 35,000 / 25 $\approx$ &dollar 1,400 per report.

*Human Intervention Cost*
* 2 of 25 reports require significant rewrites $\Rightarrow$ **Intervention penalty**: We assign a governance penalty of &dollar 15 per basis point of intervention; hence 700 b.p. =7\% $\times$ &dollar 15 = &dollar 10,500.
* Policy gate: 1% (100 b.p.) $\Rightarrow$ excess 700 b.p.

*Agent Economic Value (AEV)*
* Model Cost (inference tokens + premium): &dollar 400.
* Data Cost (alt-data subscription slices): &dollar 150.
* Intervention penalty ($\lambda$ = 15 b.p.): &dollar 10,500.
* **AEV** = &dollar 12,560 + &dollar 1,400 - (&dollar 400 + &dollar 150) - &dollar 10,500 = &dollar 2,910 per report.

**Key Outcomes**
Even after a heavy intervention penalty, the agent achieves a 20% profit margin per deep-research report while trebling research coverage. Reducing the intervention rate to 2\% would lift AEV above &dollar 10,000, offering a clear ROI target for further reliability work.
